...
The SPA at my.lifetime.life talks to api.lifetimefitness.com through Azure
API Management. Every request needs:
    - ocp-apim-subscription-key: static app-level key, same for all users
    - x-ltf-jwe:   opaque per-session auth blob (JWE-encrypted)
    - x-ltf-profile: signed JWT whose payload includes memberId/partyId
    - x-ltf-ssoid: Azure B2C SSO session id
    - x-timestamp: ISO-8601 request time
Those per-user tokens are populated by the SPA after Azure B2C login, so the
bot captures them out of the browser post-login and replays them here for
all subsequent calls (schedule search, reservation, waitlist).
"""
...
class SessionTokens:
    """Credentials lifted from the browser after Selenium login."""
        ...
        self.session.headers.update(
            {
                ...
                "x-ltf-jwe": tokens.jwe,
                "x-ltf-profile": tokens.profile,
                "x-ltf-ssoid": tokens.ssoid,
            }
        )
    ...
    def list_classes(
        ...
        *,
        ...
        for interest in interests or []:
            params.append(("tags", f"interest:{interest}"))
...
def _parse_registration_result(payload: dict[str, Any]) -> RegistrationResult:
    reg_id = _first_int(payload, "id", "registrationId", "registration_id")
    if reg_id is None:
        raise LifetimeAPIError(
